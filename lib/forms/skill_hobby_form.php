<?php
	...
	if($EmpID!='00000'){
		...
		$InputState="";
		if($mode==-1){
			$result=$MySQLi->sqlQuery("SELECT * FROM `tblempskills` WHERE `EmpID`='".$EmpID."' AND `SkillID`='".$SkillID."' LIMIT 1;");
...
<center>
	<form name="f_skill_info" onSubmit="processForm('skil',this);return false;"><br/>
		<table class="form_window">	<?php //Spouse"s Name... ?>
			<tr>
				<td class="form_label" style="width:200px;"><label>SPECIAL SKILL OR HOBBY: </label></td>
				<td class="pds_form_input"><input value="<?php echo $records['SkillDesc']; ?>" type="text" name="SkillDesc" id="SkillDesc" class="text_input sml_frm_fld" <?php echo $InputState; ?> <?php echo $InputState; ?> /></td>
			</tr>
		</table>
		<br/>
