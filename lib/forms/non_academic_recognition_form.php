<?php
	...
	$NonAcadRecID=isset($_POST['xid']) ? trim(strip_tags($_POST['xid'])) : '';
	...
	if($EmpID!='00000'){
		...
		$InputState="";
		if($mode==-1){
			$result=$MySQLi->sqlQuery("SELECT * FROM `tblempnonacadrecognitions` WHERE `EmpID`='".$EmpID."' AND `NonAcadRecID`='".$NonAcadRecID."' LIMIT 1;");
			$records=mysqli_fetch_array($result, MYSQLI_BOTH);
			$InputState="disabled";
		}
		else if($mode==0){
			$records['NonAcadRecDetails']="";
		}
		else if($mode==1){
			$result=$MySQLi->sqlQuery("SELECT * FROM `tblempnonacadrecognitions` WHERE `EmpID`='".$EmpID."' AND `NonAcadRecID`='".$NonAcadRecID."' LIMIT 1;");
			$records=mysqli_fetch_array($result, MYSQLI_BOTH);
...
<center>
	<form name="f_dpnt_info" onSubmit="processForm('ncad',this);return false;"><br/>
		<table class="form_window">	<?php //Spouse"s Name... ?>
			<tr>
				<td class="form_label" style="width:200px;"><label>NON-ACADEMEC RECOGNITION: </label></td>
				<td class="pds_form_input"><input value="<?php echo $records['NonAcadRecDetails']; ?>" type="text" name="NonAcadRecDetails" id="NonAcadRecDetails" class="text_input sml_frm_fld" <?php echo $InputState; ?> <?php echo $InputState; ?> /></td>
			</tr>
		</table>
		<br/>
		...
		<input type="hidden" name="NonAcadRecID" id="NonAcadRecID" value="<?php echo $NonAcadRecID; ?>" />
