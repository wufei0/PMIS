<?php
	...
	switch ($st){
		case -1	: 	// send a simple odbc query. returns an odbc cursor // employment_status = '".$EmploymentStatus."' AND // ".($EmploymentStatus=='A'?"ACTIVE":($EmploymentStatus=='D'?"DEAD FILE":($EmploymentStatus=='I'?"INACTIVE":"ON LEAVE")))."
					$sql="SELECT count(*) AS Personnels FROM personal WHERE pers_id >= '".$StartID."' AND pers_id <= '".$EndID."' ORDER BY pers_id ASC";
		...
		case 0	:
					...
					$sql="SELECT pers_id, last_name, middle_name, first_name, telno, birthdate, birthplace_prov, birthplace_muni, sex, marital_status, height, weight, blood_tye, gsis_no, sss_no, address_street_no, address_bgy, address_muni, address_prov, email_address, tin, spouse, spouse_occupation FROM personal WHERE pers_id='".$curID."'";
					$cur=odbc_exec($cnx, $sql);
					if(!$cur){echo "-1|0|0|0|0|ERROR 49:~Error in odbc_exec(no cursor returned) $sql";odbc_close($cnx);exit();}
					$Personal=odbc_fetch_array($cur);
					$EmpID=(strlen(strtoupper(trim($Personal['pers_id'])))?"0".strtoupper(trim($Personal['pers_id']))==4):strtoupper(trim($Personal['pers_id']));
					...
					$BirthPlaceMuni=(strlen(trim($MySQLi->RealEscapeString($BPlace['municipality'])))>6)?substr(trim($MySQLi->RealEscapeString($BPlace['municipality'])), 0, -5).", ":"";
					/* Get Birth Place - PROVINCIAL */
					$Place=odbc_exec($cnx, "SELECT province FROM province WHERE prov_code = '".$Personal['birthplace_prov']."'");
					if(!$Place){echo "-1|0|0|0|0|ERROR 49:~Error in odbc_exec(no cursor returned) $sql";odbc_close($cnx);exit();}
					$BPlace=odbc_fetch_array($Place);
					$BirthPlaceProv=$MySQLi->RealEscapeString(strtoupper(trim($BPlace['province'])));
					...
					if(!$Place){echo "-1|0|0|0|0|ERROR 49:~Error in odbc_exec(no cursor returned) $sql";odbc_close($cnx);exit();}
					$Add=odbc_fetch_array($Place);
					$Add_muni=$MySQLi->RealEscapeString($Add['municipality']);
					$EmpPerAddMun=$EmpResAddMun=$EmpSpsAddMun=$MySQLi->RealEscapeString(substr(trim($Add_muni), 0, -5));
					$EmpPerZipCode=$EmpResZipCode=$EmpSpsZipCode=$MySQLi->RealEscapeString(substr(trim($Add_muni), -4, 4));
					...
					$EmpMobile=$MySQLi->RealEscapeString(strtoupper(trim($Personal['telno'])));
					$EmpAgencyNo=$EmpID;
					...
					if($MySQL->NumberOfRows("SELECT `EmpID` FROM `tblemppersonalinfo` WHERE `EmpID`='".$EmpID."';")==0){
					...
					if($DoneIDs>=$TotalRecords){
						...
						/* Get Next ID */
						$sql="SELECT pers_id FROM personal WHERE pers_id > '".$curID."' AND pers_id >= '".$StartID."' AND pers_id <= '".$EndID."' ORDER BY pers_id ASC"; 
						$cur=odbc_exec($cnx,$sql);
						if(!$cur){echo "-1|0|0|0|0|ERROR 49:~Error in odbc_exec(no cursor returned) $sql";odbc_close($cnx);exit();}
