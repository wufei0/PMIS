	...
	<div style="width:640px;height:auto;overflow:auto"><br/><br/>
		<table class="i_table" style="width:600px;">
			<tr>
				...
				<td class='i_table_header'>Membership in Association/Organization</td>
				<td class="i_table_header" width="40px" colspan="2">&nbsp;</td>
			</tr>
			<?php
				$records=Array();
				$result=$MySQLi->sqlQuery("SELECT * FROM `tblempassorgmembership` WHERE `EmpID`='".$EmpID."' ORDER BY `MemAssOrgDesc` ASC;");
				$n=1;$td=2;
				while($records=mysqli_fetch_array($result, MYSQLI_BOTH)){
					...
					if($Authorization[0]&&$Authorization[3]){echo "<td style='width:20px;text-align:center;padding:2px 3px 1px 0px;'><ul class='ui-widget ui-helper-clearfix ul-icons'><li class='ui-state-default ui-corner-all' title='Delete' onClick='showForm(\"orgs\",\"$EmpID\",\"".$records['MemAssOrgID']."\",\"-1\");'><span class='ui-icon ui-icon-trash'></span></li></ul></td>";}
				...
				if($Authorization[0]&&$Authorization[2]){$bAddState="";$bAddClass="button ui-button ui-widget ui-corner-all";$onClick="showForm('orgs','$EmpID','',0);";}
			?>
		</table>
